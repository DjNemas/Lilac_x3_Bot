...
namespace Lilac_x3_Bot.ExtraFeatures
{
    class ListenFor1337
    {
        ...
        private async Task Message1337(SocketMessage rawMessage)
        {
            ...
            // Check if it is the right time! 133700 - 133759
            if (timeBeginAsInt <= timeNowAsInt && timeEndAsInt > timeNowAsInt)
            {
                ...
                if (counter1337 == 1 && counterM1337 != 1 || counter1337 != 1 && counterM1337 == 1)
                {
                    ...
                    if (userID.Count() == 0)
                    {
                        ...
                        if (_devMode) await context.Channel.SendMessageAsync(context.Guild.GetUser(message.Author.Id).Mention + " Hab dich gezählt :P");
                    }
                    else // Update user if allready exist
                    {
                        ...
                        {
                            if (_devMode) await context.Channel.SendMessageAsync("Du wurdest heute schon gezählt. Schummeln gilt nicht!! <:pandabulle:327873024017563649>");
                        }
            ...
            else // can be removed later
            {
                if (_devMode) await context.Channel.SendMessageAsync("Nicht in der richtigen Zeit!");
            }
            // Beispiel
